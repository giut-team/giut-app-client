...
export const detailsByProfileId: Record<string, ProfileDetail> = {
  "kim-hyeonjin": { skills: ["Spring", "Java", "MySQL", "API 설계"], portfolios: portfolio(["공공데이터 기반 민원 서비스", "팀 매칭 API 설계", "교내 해커톤 서버 개발", "데이터 연동 프로젝트"], ["공공데이터를 활용한 민원 서비스의 백엔드를 구현했어요", "팀원 추천과 지원 현황을 위한 API를 설계했어요", "서버 개발과 배포 환경 구성을 맡았어요", "외부 데이터 API를 서비스에 연결했어요"]) },
  "park-seojun": { skills: ["Python", "SQL", "데이터 분석", "데이터 시각화"], portfolios: portfolio(["상권 데이터 분석 프로젝트", "교내 만족도 분석", "공모전 데이터 대시보드", "수요 예측 모델 실험"], ["지역별 상권 데이터를 분석해 인사이트를 도출했어요", "설문 결과를 정리하고 핵심 지표를 시각화했어요", "공모전 발표용 데이터 대시보드를 만들었어요", "데이터 기반 수요 예측 모델을 실험했어요"]) },
  "choi-yuna": { skills: ["UI 디자인", "Figma", "프로토타이핑", "디자인 시스템"], portfolios: portfolio(["캠퍼스 앱 UI 리디자인", "공모전 서비스 랜딩 페이지", "모바일 디자인 시스템", "사용자 여정 개선"], ["복잡한 정보를 쉽게 찾을 수 있는 앱 화면을 설계했어요", "서비스의 첫인상을 전달하는 랜딩 페이지를 디자인했어요", "반복해서 활용할 수 있는 UI 컴포넌트를 정리했어요", "사용자 흐름을 분석해 핵심 경험을 개선했어요"]) },
...
const activitiesByProfileId: Record<string, Activity[]> = {
  "kim-hyeonjin": [{ title: "공공데이터 활용 해커톤 우수상", description: "백엔드 · 4인 팀" }, { title: "개발 동아리 서버 파트", description: "Spring · PostgreSQL" }],
  "park-seojun": [{ title: "데이터 분석 공모전 본선 진출", description: "데이터 분석 · 3인 팀" }, { title: "통계학과 데이터 스터디", description: "Python · SQL" }],
  "choi-yuna": [{ title: "교내 UX/UI 디자인 공모전 수상", description: "UI 디자인 · 3인 팀" }, { title: "모바일 앱 디자인 스터디", description: "Figma · 프로토타이핑" }],
...
function ProfileShareSheet({
  ...
  return (
    <BottomSheet minHeight="auto" onClose={onClose} open={open} showHeaderDivider={false} variant="compact">
      <S.ShareHeading>프로필 공유</S.ShareHeading>
      <S.ShareDescription>{profile.name} 님의 프로필을 팀원에게 공유할 수 있어요.</S.ShareDescription>
      <S.ShareLinkBox><code>{profileLink}</code><S.CopyButton onClick={copyProfileLink} type="button">{isCopied ? "복사됨" : "복사"}</S.CopyButton></S.ShareLinkBox>
      <S.ShareChannelList aria-label="공유 방식 선택">
        <S.ShareChannel onClick={openNativeShare} type="button"><S.KakaoMark><Icon name="chat" size={23} weight="fill" /></S.KakaoMark><span>카카오톡</span></S.ShareChannel>
        <S.ShareChannel onClick={copyProfileLink} type="button"><S.ShareChannelIcon><Icon name="link" size={24} weight="bold" /></S.ShareChannelIcon><span>링크 공유</span></S.ShareChannel>
        <S.ShareChannel onClick={openNativeShare} type="button"><S.ShareChannelIcon><Icon name="share" size={24} weight="bold" /></S.ShareChannelIcon><span>기타</span></S.ShareChannel>
      </S.ShareChannelList>
      <S.ShareCloseButton onClick={onClose} type="button">닫기</S.ShareCloseButton>
    </BottomSheet>
  );
}

const reportReasons = ["스팸 · 광고성 프로필", "허위 경력 · 정보 도용", "부적절한 언행 · 괴롭힘", "기타"];
...
function ProfileReportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <GiutHubDialog description="신고 사유를 선택해 주세요. 운영팀이 24시간 내 검토합니다." onClose={onClose} open={open} title="신고하기">
      <S.ReportReasonList aria-label="신고 사유 선택">
        {reportReasons.map((reason) => <S.ReportReason $selected={selectedReason === reason} aria-pressed={selectedReason === reason} key={reason} onClick={() => setSelectedReason(reason)} type="button"><S.ReportRadio $selected={selectedReason === reason} />{reason}</S.ReportReason>)}
      </S.ReportReasonList>
      <S.DangerDialogActions>
        <S.DialogCancelButton onClick={onClose} type="button">취소</S.DialogCancelButton>
        <S.DialogDangerButton onClick={onClose} type="button">신고 접수</S.DialogDangerButton>
      </S.DangerDialogActions>
    </GiutHubDialog>
  );
}

function ProfileBlockModal({ open, profileName, onClose }: { open: boolean; profileName: string; onClose: () => void }) {
  return (
    <GiutHubDialog
      description={`차단하면 서로의 프로필과 팀 제안이 노출되지 않아요.\n설정에서 언제든 해제할 수 있어요.`}
      ...
      tone="danger"
    >
      <S.BlockNotice><span>· 진행 중인 팀 제안 1건이 자동 취소됩니다</span><span>· 주고받은 채팅방이 숨겨집니다</span></S.BlockNotice>
      <S.DangerDialogActions>
        <S.DialogCancelButton onClick={onClose} type="button">취소</S.DialogCancelButton>
        <S.DialogDangerButton onClick={onClose} type="button">차단하기</S.DialogDangerButton>
      </S.DangerDialogActions>
    </GiutHubDialog>
  );
}
...
export function GiutHubProfilePage() {
  ...
  const [profileAction, setProfileAction] = useState<"share" | "report" | "block" | null>(null);
  ...
  return (
    <S.Page>
      ...
      <ProfileMoreBottomSheet onBlock={() => openProfileAction("block")} onClose={() => setIsMoreSheetOpen(false)} onReport={() => openProfileAction("report")} onShare={() => openProfileAction("share")} open={isMoreSheetOpen} />
      <ProfileShareSheet onClose={() => setProfileAction(null)} open={profileAction === "share"} profile={profile} />
      <ProfileReportModal onClose={() => setProfileAction(null)} open={profileAction === "report"} />
      <ProfileBlockModal onClose={() => setProfileAction(null)} open={profileAction === "block"} profileName={profile.name} />
    </S.Page>
  );
}
