...
function PortfolioShareModal({
  ...
  const openNativeShare = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: `${portfolioTitle} 포트폴리오`, text: `${portfolioTitle} 포트폴리오를 확인해 보세요.`, url: window.location.href });
      } else {
  ...
  return (
    <GiutHubDialog description={`'${portfolioTitle}' 페이지 링크를 공유해요.`} onClose={onClose} open={open} title="포트폴리오 공유">
      <S.SharePreview><S.SharePreviewThumbnail /><span><strong>{portfolioTitle}</strong><small>{profileName} · {detailRole}</small></span></S.SharePreview>
      <S.ShareLinkBox><code>{portfolioLink}</code><S.CopyButton onClick={copyPortfolioLink} type="button">{isCopied ? "복사됨" : "복사"}</S.CopyButton></S.ShareLinkBox>
      <S.ShareChannelList aria-label="공유 방식 선택">
        <S.ShareChannel onClick={openNativeShare} type="button"><S.KakaoMark><Icon name="chat" size={22} weight="fill" /></S.KakaoMark><span>카카오톡</span></S.ShareChannel>
        <S.ShareChannel onClick={copyPortfolioLink} type="button"><S.ShareChannelIcon><Icon name="link" size={23} weight="bold" /></S.ShareChannelIcon><span>링크 공유</span></S.ShareChannel>
        <S.ShareChannel onClick={openNativeShare} type="button"><S.ShareChannelIcon><Icon name="share" size={23} weight="bold" /></S.ShareChannelIcon><span>기타</span></S.ShareChannel>
      </S.ShareChannelList>
    ...
    <S.Page $isLeaving={isLeaving}>
      <S.Content onPointerDown={handleSwipeStart} onPointerUp={handleSwipeEnd}>
        <PageHeader
          centerTitle
          onBack={goBack}
          rightContent={<S.ShareButton aria-label="공유하기" onClick={() => setIsShareModalOpen(true)} type="button"><Icon name="share" size={25} weight="regular" /></S.ShareButton>}
          title="포트폴리오"
        />
      ...
      <PortfolioShareModal detailRole={profile.detailRole} onClose={() => setIsShareModalOpen(false)} open={isShareModalOpen} portfolioTitle={portfolio.title} profileName={profile.name} />
    </S.Page>
  );
}
